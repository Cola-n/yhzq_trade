// code_generator_tag
// messages:624852200dc84ca805299ecd357d9a28 template:d5e676b48a16d8942e0c6cea656a6229 code:2633190dd3091959ff71aad3990f22e2
...
struct TRADE_API ATPReqCashAuctionOrderMsg
{
    ...
    ATPPriceType price; ///委托价格N13(4) （上海市场股票、存托凭证、基金市价委托必须输入保护价）
    ...
    ATPBatchClOrdNoType batch_cl_ord_no; ///客户自定义委托批号（0为系统保留值，不允许使用）（ATP V3.2.3版本开始支持）
    ATPOrderWayExtType order_way_ext; ///该字段为“扩位委托方式”字段，仅在客户集中交易柜台的委托方式编码大于1位时启用，如柜台不支持或支持但未启用请勿填入该字段（ATP V3.2.10版本开始支持）
...
struct TRADE_API ATPReqCancelOrderMsg
{
    ...
    ATPBatchClOrdNoType batch_cl_ord_no; ///客户自定义委托批号，与orig_cl_ord_no至少填写一项。若orig_cl_ord_no不为空，则按orig_cl_ord_no撤单；否则，batch_cl_ord_no不为空，按batch_cl_ord_no撤单(ATP V3.2.3版本开始支持)
    ATPOrderWayExtType order_way_ext; ///该字段为“扩位委托方式”字段，仅在客户集中交易柜台的委托方式编码大于1位时启用，如柜台不支持或支持但未启用请勿填入该字段（ATP V3.2.10版本开始支持）
...
struct TRADE_API ATPRspOrderStatusAckMsg
{
    ...
    ATPOrdRejReasonType ord_rej_reason; ///交易所/ORS、COS返回的拒绝原因代码。当ORS/COS拒绝时，可调用GetErrorInfo错误码查询函数，传入错误码及错误类型-1 ORS/COS拒绝原因错误码类型,即可查询错误描述信息。
    ATPOrdTypeType order_type; ///订单类型
    ...
    ATPBatchClOrdNoType batch_cl_ord_no; ///客户自定义委托批号，对于撤单返回响应，为原订单的客户自定义委托批号(ATP V3.2.3版本开始支持)
...
struct TRADE_API ATPRspCashAuctionTradeERMsg
{
    ...
    ATPParticularFlagsType particular_flags; ///特殊委托标识，右起第一个bit位是夜市委托标志位：0-正常委托、1-夜市委托（ATP3.2.0开始支持）
    ATPBatchClOrdNoType batch_cl_ord_no; ///客户自定义委托批号(ATP V3.2.3版本开始支持)
    ATPAmtType net_total_value_traded; ///成交金额（含利息）N18(4)(ATP V3.2.3.2版本开始支持)
...
struct TRADE_API ATPReqOrderQueryMsg
{
    ...
    BusinessAbstractType business_abstract_type; ///抽象业务编码 
    ATPBatchClOrdNoType batch_cl_ord_no; ///客户自定义委托批号（默认值为0，表示查所有）（ATP 3.2.3版本开始支持）
...
struct TRADE_API ATPReqBatchOrderQueryMsg
{
    ...
    ATPReportIndexType query_index; ///起始查询索引号 （默认值为0，表示从头开始查）
    ATPTotalNumType return_num; ///查询返回数量 （默认值为0，表示按能返回的最大数量返回，具体数量请咨询券商）
    ATPOrderQueryConditionType order_query_condition; ///委托查询条件 （默认值为0，表示查所有，一户两地账户表示查单个证券账户下所有）(ATP V3.2.3.2版本开始支持)
...
struct TRADE_API ATPReqTradeOrderQueryMsg
{
    ...
    ATPTotalNumType return_num; ///查询返回数量 （默认值为0，表示按能返回的最大数量返回，具体数量请咨询券商）
    ATPReturnSeqType return_seq; ///返回顺序，按时间排序 （默认值为1，表示按时间正序排序）
    BusinessAbstractType business_abstract_type; ///抽象业务编码（无意义）
    ATPClOrdNOType cl_ord_no; ///客户订单编号
    ...
    ATPBatchClOrdNoType batch_cl_ord_no; ///客户自定义委托批号（默认值为0，表示查所有）（ATP 3.2.3版本开始支持）
    ATPTradeOrderQueryConditionType trade_order_query_condition; ///成交查询条件 （默认值为0，表示按全局参数设置的模式进行查询）（ATP 3.2.8版本开始支持）
...
struct TRADE_API ATPReqShareQueryMsg
{
    ...
    ATPBusinessTypeType business_type; ///业务类型（无意义）
...
struct TRADE_API ATPRspMaxOrderQueryResultMsg
{
    ...
    ATPUserInfoType user_info; ///用户私有信息
    ATPExtraData extra_data; ///扩展字段
    ATPQtyType max_order_qty; ///最大可委托数（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）N15(2)（开放式基金申购，此字段为最大可申购金额。）
    ATPQtyType theoretical_order_qty; ///理论可委托数（股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张）N15(2)（开放式基金申购，此字段为最大可申购金额。）
...
struct TRADE_API APIOrderUnit
{
    ...
    ATPParticularFlagsType particular_flags; ///特殊委托标识，右起第一个bit位是夜市委托标志位：0-正常委托、1-夜市委托（ATP3.2.0开始支持）
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（ATP3.2.3 开始支持）
    ATPBatchClOrdNoType batch_cl_ord_no; ///客户自定义委托批号（对于撤单订单，为原订单的客户自定义委托批号）(ATP V3.2.3版本开始支持)
    ATPUserInfoType user_info; ///用户私有信息(ATP V3.2.3.2版本开始支持)
...
struct TRADE_API APITradeOrderUnit
{
    ...
    ATPClOrdNOType cl_ord_no; ///客户订单编号（若为撤单成功，为原订单编号；若为撤单失败，为撤单订单编号）
    ATPOrderIDType order_id; ///交易所订单编号（对于交易所确认撤单的委托，order_id为空）
    ATPClOrdIDType cl_ord_id; ///申报合同号（上交所：以QP1开头,表示为交易所保证金强制平仓；以CV1开头,表示为交易所备兑强制平仓）
    ATPTimeStampType transact_time; ///成交时间（对于交易所确认撤单的委托，取交易所确认撤单的回报时间）
    ATPPriceType last_px; ///成交价格N13(4)（对于交易所确认撤单的委托，取0；对于ETF实时申赎成交，取0）
    ...
    ATPParticularFlagsType particular_flags; ///特殊委托标识，右起第一个bit位是夜市委托标志位：0-正常委托、1-夜市委托（ATP3.2.0开始支持）
    ATPBatchClOrdNoType batch_cl_ord_no; ///客户自定义委托批号（对于撤单成交，为原订单的客户自定义委托批号）(ATP V3.2.3版本开始支持)
    ATPAmtType net_total_value_traded; ///成交金额（含利息）N18(4)（对于交易所确认撤单的委托，取0）（ATP3.2.3.2 开始支持）
    ATPBoolType cancel_flag; ///撤单标识：0-非撤单，1-撤单
...
struct TRADE_API APIShareUnit
{
    ...
    ATPQtyType first_day_pos; ///日初持仓量（ATP V3.2.9开始支持）
    ATPStringType extra_info; ///附加信息。当证券类型为【27-配售权证】，此字段返回配售类型：0-配股，1-配债，2-减持配售；否则为空。数据格式如：{“rights_issue_type”：“0”}。（ATP V3.2.9开始支持）
...
{
    ATPSeqNumType client_seq_id; ///用户系统消息序号 
    std::vector<APINoPartitionUnit> no_parition_array; ///消息分区记录号信息，当前系统仅有一个消息分区，分区号为1
...
struct TRADE_API ATPReqCancelBatchOrderMsg
{
    ...
    ATPExtraData extra_data; ///扩展字段 
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPClOrdNOType parent_cl_ord_no; ///批量订单母单编号，指被撤消订单的parent_cl_ord_no
    ATPOrderWayExtType order_way_ext; ///该字段为“扩位委托方式”字段，仅在客户集中交易柜台的委托方式编码大于1位时启用，如柜台不支持或支持但未启用请勿填入该字段（ATP V3.2.10版本开始支持）
...
{
    ATPExtraData extra_data; ///扩展字段 
    ATPCustIDType cust_id; ///客户号ID（cust_id、fund_account_id两者必选填其一）
    ATPFundAccountIDType fund_account_id; ///资金账户ID（cust_id、fund_account_id两者必选填其一）
    ATPBranchIDType branch_id; ///营业部ID
...
struct TRADE_API ATPReqCashBankSecuritiesFundTransferOtherMsg
{
    ...
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPOrderWayType order_way; ///委托方式（是否必填以及字典项请咨询券商）
    ATPPasswordType fund_password; ///资金密码（该字段是否必填请咨询券商）
    ATPPasswordType bank_password; ///银行密码（该字段是否必填请咨询券商）
    ATPOrderWayExtType order_way_ext; ///该字段为“扩位委托方式”字段，仅在客户集中交易柜台的委托方式编码大于1位时启用，如柜台不支持或支持但未启用请勿填入该字段（ATP V3.2.10版本开始支持）
...
struct TRADE_API ApiSecurityInfoArray
{
    ...
    ATPQtyType buy_qty_unit; ///限价买数量单位N15(2)(ATP3.2.13开始支持)
    ATPQtyType sell_qty_unit; ///限价卖数量单位N15(2)(ATP3.2.13开始支持)
    ATPQtyType market_buy_qty_unit; ///市价买数量单位N15(2)(ATP3.2.13开始支持)
    ATPQtyType market_sell_qty_unit; ///市价卖数量单位N15(2)(ATP3.2.13开始支持)
    ATPNoProfitType no_profit; ///是否尚未盈利
    ATPWeightedVotingRightsType weighted_voting_rights; ///是否存在表决权差异
    ATPIsRegistrationType is_registration; ///是否注册制
    ATPIsVieType is_vie; ///是否具有协议控制架构
...
struct TRADE_API ATPRspAlgoParentExecReportMsg
{
    ...
    ATPIndexType index; ///回报记录号
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（ATP3.2.3开始支持）
